...
jQuery("document").ready(function () {
    ...
    jQuery("#two_delay_all_js_execution").change(function () {
        if (this.checked) {
            ...
            jQuery(".two_timeout_js_load,.two_exclude_rev,.two_exclude_slider_by_10web,.two_exclude_elementor_scripts").css({
                'display': 'block'
            });
        } else {
            jQuery(".two_js_options").css({
    ...
    jQuery(".two_delete_clear_cache_logs, .two_delete_critical_css_logs").on("click", function (e) {

        let btn = this;
        let btn_text = btn.textContent;
        btn.textContent = "Deleting";
        let task = (btn.classList.contains("two_delete_clear_cache_logs")) ? "delete_clear_cache_logs" : "delete_critical_css_logs";
        ...
        jQuery.ajax({
            ...
            if (data.success) {
                let th_elements_to_remove;
                if(task == "delete_clear_cache_logs"){
                    th_elements_to_remove = jQuery("table.two_clear_cache_logs tbody th");
                }else{
                    th_elements_to_remove = jQuery("table.two_critical_css_logs tbody th");
                }
                for(let el of th_elements_to_remove){
                    el.remove();
                }
            } else {
