...
class OptimizerCriticalCss
{
    ...
    const LOG_OPTION_NAME = "two_critical_css_logs";
        ...
        try {
          ...
          set_transient($critical_in_progress_key, "1" , 30 * MINUTE_IN_SECONDS);
            ...
            if( !TENWEB_SO_HOSTED_ON_10WEB ){
                ...
                if(empty($flow_id) || empty($notification_id)){
                ...
                if($access_token && $domain_id){
                    ...
                    delete_site_option(TENWEB_PREFIX . '_flow_id');
                    delete_site_option(TENWEB_PREFIX . '_notification_id');
                    self::log_critical_generation_data($request_data, $newly_connected_website,$flow_id, wp_remote_retrieve_response_code($res));
                }
                ...
                self::log_critical_generation_data($request_data, $newly_connected_website, get_site_option(TENWEB_PREFIX . '_flow_id'), wp_remote_retrieve_response_code($response));
        ...
        if (is_array($data["page_sizes"]) && isset($tenweb_domain_id)) {
            if(empty($data["page_sizes"])){
                ...
                if(isset($default_settings["two_critical_sizes"]) && !empty($default_settings["two_critical_sizes"])){
                    $data["page_sizes"] = $default_settings["two_critical_sizes"];
                }
            }
  ...
  private static function log_critical_generation_data($request_data, $newly_connected_website, $flow_id, $status_code){
    $log_data = [
    ...
    OptimizerAdmin::add_log(self::LOG_OPTION_NAME, $log_data);
  }
