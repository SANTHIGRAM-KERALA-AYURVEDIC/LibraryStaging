...
class OptimizerUtils
{
  ...
  public static function get_post_id($page_url = null) {
      ...
      if( !empty( $page_url ) ) {
          ...
          return self::get_current_post_info();
      }
  }
            ...
            if ( !is_wp_error( $response )  && isset($response["body"], $response["response"]["code"]) && $response["response"]["code"] === 200) {
                ...
                if ( $response_body["data"]["success"] ) {
                    ...
                    if (isset($response_body["data"]["plan_title"])) {
                        $plan_title = $response_body["data"]["plan_title"];
                        $tenweb_plan_title = $plan_title;
                        set_transient(TENWEB_PREFIX . '_plan_title', $plan_title, 12 * HOUR_IN_SECONDS);
                    }
                }
            }else{
                set_transient(TENWEB_PREFIX . '_subscription_id', "0", 1 * HOUR_IN_SECONDS);
                set_transient(TENWEB_PREFIX . '_plan_title', "", 1 * HOUR_IN_SECONDS);
            }
            return array( 'tenweb_subscription_id' => $tenweb_subscription_id, 'tenweb_plan_title' => $tenweb_plan_title );
        }
  }
